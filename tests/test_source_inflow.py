...
def test_exp01_single_heavy_source_inflow_uniform_bath_with_embedded_defect_ramp(tmp_path) -> None:
    ...
    bath_density = 2.0 / 12.0**3
    config = {
        "run_name": "exp01_single_heavy_source_inflow_uniform_bath_ramped_defect_test",
        "seed": 1234,
        "device": "cpu",
        "dtype": "float32",
        "complex_dtype": "complex64",
    ...
    config_path = tmp_path / "config_uniform_bath_ramped_defect.json"
    config_path.write_text(json.dumps(config), encoding="utf-8")
    ...
    assert summary["initializer_mode"] == "uniform_bath"
    assert summary["embedded_defect_enabled"] is True
    assert summary["embedded_defect_ramp_enabled"] is True
