...
| Live heavy-source inflow calibration | `active` | Prepared as the next source-sector branch: a single centered heavy defect with sparse shell-flux, density, leakage, and coherence diagnostics before introducing a second body |
...
- Run the new single-heavy-source calibration branch on the A40:
  - `./scripts/run_exp01_single_heavy_source_inflow_320_cuda.sh`
- Use that run to measure whether a centered heavy defect reaches a stable inflow/source state with acceptable coherence and low bulk density drift before adding a second body.
- If the single-source branch is interpretable, the next branch should add a light defect and test radial infall against the Newtonian `1/r^2` oracle using the same live-source protocol.
